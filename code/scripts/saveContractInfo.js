...
async function writeAbiAddr(artifacts, addr, network) {
    ...
    writeLog(deployments, artifacts.contractName, network);
    const abis = {};
    ...
    return writeFileGrace(deploymentPath, abis);
}
...
async function writeFileGrace(filePath, data) {
    ...
    return writeFile(filePath, JSON.stringify(data, null, 2), { flag: 'w' });
}
