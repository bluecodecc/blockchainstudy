...
const SCORE_CONSTRACT_INFO = require(`../deployments/${network.name}/Score.json`)
const TEACHER_CONSTRACT_INFO = require(`../deployments/${network.name}/Teacher.json`)
async function main() {
    let [owner] = await ethers.getSigners();
    let teacherContract = await ethers.getContractAt(TEACHER_CONSTRACT_INFO.contractName, TEACHER_CONSTRACT_INFO.address, owner);
    let scoreContract = await ethers.getContractAt(SCORE_CONSTRACT_INFO.contractName, SCORE_CONSTRACT_INFO.address, owner);
    //teacher合约所有者或者techer可以修改score
    // await teacherContract.updateScore(SCORE_CONSTRACT_INFO.address, owner.address, 96);
    console.log((await scoreContract.scores(owner.address)).toString());
    //非teacher合约所有者或者非teacher不可以修改score
    // await scoreContract.updateScore(owner.address, 1);
    // console.log((await scoreContract.scores(owner.address)).toString());
}
main()
    .then(() => process.exit(0))
